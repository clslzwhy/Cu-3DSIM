...
#include "mclmcrrt.h"
#include "libMatdeconvlucy.h"
#include "libMatlab_edgetaper.h"
#include "libCalMCNR.h"
#include "libFFT2D.h"
#include "libGenHilo.h"
...
public:
	...
	void deconvlucy(double* result, double* src, double* psf, int dataH, int dataW, int Nz);
	void edgetaper(std::vector<cv::Mat>& Snoisy);
	void Mat_edgetaper(cv::Mat& src);
	void otf2psf(double* psf, double* otf, int ImgSize,sCudaPara& para);
	mxArray* TifMat2mwArray_Double(cv::Mat src);
	mxArray* CeviceDouble2mwArray_Double(double* src, int dataH, int dataW, int Nz);
	mxArray* MatReal_2mwArray_Double(cv::Mat src);
	mxArray* Double2mwArray(double Num);
	void MuitymwArry2Devicedouble(double* output, mxArray* QQ, int dataH, int dataW, int Nz);
	void importImages(double* dst, double* src, sCudaPara& para);
	...
	void Cal_MCNR(double* result, std::vector<cv::Mat>& Snoisy, int  dataH, int dataW, int Nz);
	mxArray* TifMuiMat2mwArray_Double(std::vector<cv::Mat>& Snoisy);
	void Mat_FFT2D_1(cufftDoubleComplex* result, double* src_real, double* src_imag, int dataH, int dataW, int Nz);

	void CeviceComplex2mwArray_Double(mxArray* dst_real, cufftDoubleComplex* src, int dataH, int dataW, int Nz);
	void Mat_HiloGen(double* result, std::vector<cv::Mat>& Snoisy, int numpixelsx1, int Nz, double Na, double emwavelength, double rawpixelsize);
	int Npola;
	int Nazim;//3不是Nz
