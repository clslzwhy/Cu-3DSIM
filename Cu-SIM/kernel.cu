...
__global__ void Calfinal_image(double* dst, double* src1, double* src2, int Raw_Size_Q) {
	...
	dst[i] = src1[i]/3 + src2[i];
}

__global__ void fadeBorderCos(double* dst, double* src, double fac, int px)
{


	int P_X = threadIdx.x + threadIdx.y * blockDim.x;
	int P_Y = blockIdx.x + blockIdx.y * gridDim.x;
	int P_Z = blockIdx.z * gridDim.y * gridDim.x * blockDim.x * blockDim.y;
	int i0 = P_X + (blockDim.x * blockDim.y) * P_Y;
	int i = i0 + P_Z;

	if (P_Y < px && P_X < px)
	{
		dst[i] = src[i] * pow(sin((P_Y)*fac), 2.0) * pow(sin((P_X)*fac), 2.0);
	}
	else if (P_Y > (gridDim.y * gridDim.x - px - 1) && P_X > (blockDim.x * blockDim.y - px - 1))
	{
		dst[i] = src[i] * pow(sin((gridDim.y * gridDim.x - P_Y - 1) * fac), 2.0) * pow(sin((blockDim.x * blockDim.y - P_X - 1) * fac), 2.0);
	}
	else if (P_Y > (gridDim.y * gridDim.x - px - 1) && P_X < px)
	{
		dst[i] = src[i] * pow(sin((gridDim.y * gridDim.x - P_Y - 1) * fac), 2.0) * pow(sin((P_X)*fac), 2.0);
	}
	else if (P_Y < px && P_X >(blockDim.x * blockDim.y - px - 1))
	{
		dst[i] = src[i] * pow(sin((P_Y)*fac), 2.0) * pow(sin((blockDim.x * blockDim.y - P_X - 1) * fac), 2.0);
	}
	else if (P_Y < px)
	{
		dst[i] = src[i] * pow(sin((P_Y)*fac), 2.0);
	}
	else if (P_X < px)
	{
		dst[i] = src[i] * pow(sin((P_X)*fac), 2.0);
	}
	else if (P_Y > (gridDim.y * gridDim.x - px - 1))// 
	{
		dst[i] = src[i] * pow(sin((gridDim.y * gridDim.x - P_Y - 1) * fac), 2.0);
	}
	else if (P_X > (blockDim.x * blockDim.y - px - 1))//
	{
		dst[i] = src[i] * pow(sin((blockDim.x * blockDim.y - P_X - 1) * fac), 2.0);
	}
	else
	{
		dst[i] = src[i];
	...
	//int i = i0 + blockIdx.z * Raw_Size_Q;
...
__global__ void Matrixconj(double* dst, double* src,int Raw_Size_Q,int Nz)
{
	int threadId_2D = threadIdx.x + threadIdx.y * blockDim.x;
	int blockId_2D = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * (gridDim.x * gridDim.y);
	int i0 = threadId_2D + (blockDim.x * blockDim.y) * blockId_2D;
	...
	{
		int i = x * Raw_Size_Q + y;

		int j = y * Raw_Size_Q + x;
		

		dst[i+ Testi* Raw_Size_Q* Raw_Size_Q] = src[j +Testi * Raw_Size_Q * Raw_Size_Q];
...
__global__ void Cal_mixing_matrix(cufftDoubleComplex* dst, cufftDoubleComplex* p1, double* Module, int numangles, double numsteps)
{
	...
	//dst[i].x = patternphases;
	//dst[i].y = patternphases;
}
...
__global__ void fftShfit_R(double* dst, double* src)
{
	int threadId_2D = threadIdx.x + threadIdx.y * blockDim.x;
	int blockId_2D = blockIdx.x + blockIdx.y * gridDim.x;
	int blockId_2D0 = blockIdx.z * gridDim.y * gridDim.x * blockDim.x * blockDim.y;
...
__global__ void weightCNR(double* dst, double* src, int Nz, double* MCNR, int Raw_Size_Q)
{
	...
	//dst[i0] = src[i0 + (1 * 15) * Raw_Size_Q * Raw_Size_Q];
}
...
__global__ void Abs_R(double* dst, double* src)
{
	int threadId_2D = threadIdx.x + threadIdx.y * blockDim.x;
	int blockId_2D = blockIdx.x + blockIdx.y * gridDim.x;
	int blockId_2D0 = blockIdx.z * gridDim.y * gridDim.x * blockDim.x * blockDim.y;
...
__global__ void MatrixReal(double* dst, cufftDoubleComplex* src, int Raw_Size_Q)
{
	int threadId_2D = threadIdx.x + threadIdx.y * blockDim.x;
	int blockId_2D = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * (gridDim.x * gridDim.y);
	int i = threadId_2D + (blockDim.x * blockDim.y) * blockId_2D;
...
__global__ void MatrixImag(double* dst, cufftDoubleComplex* src,int Raw_Size_Q)
{
	int threadId_2D = threadIdx.x + threadIdx.y * blockDim.x;
	int blockId_2D = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * (gridDim.x * gridDim.y);
	int i = threadId_2D + (blockDim.x * blockDim.y) * blockId_2D;
...
void CalFuntion::ShowDouble(double* paradouble, int dataH, int dataW, int id_start, int id_end, int jd_start, int jd_end, int Z_start, int Z_end)
{
	...
	for (int ti = Z_start; ti < Z_end; ti++)
	{
		...
		cv::minMaxLoc(Retest, &minVal, &maxVal, &minLoc, &maxLoc);
		Retest = Retest / maxVal * 255;
		Retest.convertTo(Retest, CV_8UC1);
		imwrite("D:\\Test.tif", Retest);
	}
}
...
void CalFuntion::CeviceComplex2mwArray_Double(mxArray* pMat,cufftDoubleComplex* src, int dataH, int dataW, int Nz)//
{

	double* input = NULL;
	mwSize dims[2] = { dataH, dataW * Nz };
	...
	for (int ti = 0; ti < Nz; ti++)
	{
		//拷贝至内存
		cudaMemcpy(h_Result, &src[ti * dataH * dataW], dataH * dataW * sizeof(cufftDoubleComplex), cudaMemcpyDeviceToHost);
		...
		for (int i = 0; i < dataH; i++) {
			...
			for (int j = 0; j < dataW; j++)
			{
				...
				double test = h_Result[i * dataW + j].x;
				int J;
				J = dataW * ti + j;
				input[J * dataH + i] = test;
...
mxArray* CalFuntion::CeviceDouble2mwArray_Double(double* src,int dataH,int dataW,int Nz)//double Mat图片转double mwArray
{
	...
	{
		//拷贝至内存
		cudaMemcpy(h_Result, &src[ti * dataH * dataW], dataH * dataW * sizeof(double), cudaMemcpyDeviceToHost);
		...
		for (int i = 0; i < dataH; i++) {
			...
			for (int j = 0; j < dataW; j++)
			{
				...
				double test = h_Result[i * dataW + j];
				int J;
				J = dataW * ti + j;
				input[J * dataH + i] = test;
...
mxArray* CalFuntion::TifMuiMat2mwArray_Double(std::vector<cv::Mat>& Snoisy)//double Mat图片转double mwArray
{


	int h = Snoisy[0].rows;
	int w = Snoisy[0].cols;
	int c = Snoisy[0].channels();
	...
	if (c == 1) // gray image
	{
		...
		for (int k = 0; k < Nz; k++)
		{
			for (int i = 0; i < h; i++)
			{
				for (int j = 0; j < w; j++)
				{
					double test = (double)Snoisy[k].at< double>(i, j);
					int J;
					J = w * k + j;
					input[J * h + i] = test;
				}
...
void CalFuntion::MuitymwArry2Devicedouble(double* output,mxArray* QQ, int dataH, int dataW,int Nz)
{
	double* imgData = NULL;
	...
	for (int Ti = 0; Ti < Nz; Ti++)
	{
		double* h_DataComplex;//cuda复数结构体，含有x、y两个元素，x为实数，y为虚数
		h_DataComplex = (double*)malloc(dataH * dataW * sizeof(double));//分配内存
		for (int i = 0; i < dataH; i++)
		{
			subs[0] = i;
			for (int j = 0; j < dataW; j++)
			{
				...
				h_DataComplex[i] = imgData[index];
			}
		}


		cudaMemcpy(&output[Ti * dataH * dataW], h_DataComplex, dataH * dataW * sizeof(double), cudaMemcpyHostToDevice);
	}
...
void CalFuntion::Cal_MCNR(double* result, std::vector<cv::Mat>& Snoisy, int  dataH, int dataW, int Nz)
{
	libCalMCNRInitialize();

	mxArray* In1;
	mxArray* In2;
	mxArray* In3;
	mxArray* In4;
	In1 = TifMuiMat2mwArray_Double(Snoisy);

	In2 = Double2mwArray(double(dataH));
	In3 = Double2mwArray(double(dataW));
	In4 = Double2mwArray(double(Nz));
	mxArray* Out1 = mxCreateDoubleMatrix(1, Nz, mxREAL);
	mlfCal_MCNR(1, &Out1, In1, In2, In3, In4);

	cv::Mat dst;
	dst = mwArry2doubleMat(Out1);

	double total = cv::sum(dst)[0];
	dst = dst / total;
 
	//
	cudaMemcpy(&result[0], dst.data, 1 * Nz * sizeof(double), cudaMemcpyHostToDevice);


}
void CalFuntion::deconvlucy(double* result, double* src, double* psf, int dataH, int dataW, int Nz)
{
	libMatdeconvlucyInitialize();

	mxArray* In1 = CeviceDouble2mwArray_Double(src, dataH, dataW, Nz);

	mxArray* In2 = CeviceDouble2mwArray_Double(psf, dataH, dataW, 1);


	mxArray* In3;
	mxArray* In4;
	mxArray* In5;
	In3 = Double2mwArray(double(dataH));
	In4 = Double2mwArray(double(dataW));
	In5 = Double2mwArray(double(Nz));


	mxArray* Out1 = mxCreateDoubleMatrix(dataH, dataW * Nz, mxREAL);
	mxArray* Out2 = mxCreateDoubleMatrix(1, 1, mxREAL);
	mlfMatdeconvlucy(2, &Out1, &Out2, In1, In2, In3, In4, In5);


	cv::Mat dst;
	dst = mwArry2doubleMat(Out1);
	//cv::Mat dst2;
	//dst2 = mwArry2doubleMat(Out2);

	//double tse= dst.at< double>(0, 1795);
	//int tse2 = dst2.at< double>(0, 0);

	cudaMemcpy(&result[0], dst.data, dataH * dataW * Nz * sizeof(double), cudaMemcpyHostToDevice);
...
void CalFuntion::Mat_HiloGen(double* result, std::vector<cv::Mat>& Snoisy, int numpixelsx1, int Nz, double Na, double emwavelength, double rawpixelsize)
{
	libGenHiloInitialize();

	mxArray* In1 = TifMuiMat2mwArray_Double(Snoisy);

	mxArray* In2 = Double2mwArray(double(numpixelsx1));
	mxArray* In3 = Double2mwArray(double(Nz));
	mxArray* In4 = Double2mwArray(double(Na));
	mxArray* In5 = Double2mwArray(double(emwavelength));
	mxArray* In6 = Double2mwArray(double(rawpixelsize));



	mxArray* Out1 = mxCreateDoubleMatrix(numpixelsx1*2, numpixelsx1*2*Nz, mxREAL);
	//(int nargout, mxArray** Hilo, mxArray* allimages_in0, mxArray* numpixelsx1, mxArray* Nz, mxArray* Na, mxArray* emwavelength, mxArray* rawpixelsize);
	mlfGenHilo(1,&Out1, In1, In2, In3, In4, In5, In6);
	cv::Mat Mat_real;
	Mat_real = mwArry2doubleMat(Out1);


	double tse= Mat_real.at< double>(0, 0);
	




	cudaMemcpy(&result[0], Mat_real.data, numpixelsx1*4 * numpixelsx1 * Nz * sizeof(double), cudaMemcpyHostToDevice);
...
void CalFuntion::Mat_FFT2D_1(cufftDoubleComplex* result, double* src_real, double* src_imag, int dataH, int dataW, int Nz)
{
	libFFT2DInitialize();
	mxArray* In1=CeviceDouble2mwArray_Double( src_real, dataH, dataW, Nz);
	mxArray* In2 = CeviceDouble2mwArray_Double(src_imag, dataH, dataW, Nz);
	...
	mlfMat_FFT2D_1(2, &Out1, &Out2, In1, In2);


	cv::Mat Mat_real;
	Mat_real = mwArry2doubleMat(Out1);
	cv::Mat Mat_imag;
	Mat_imag = mwArry2doubleMat(Out2);

	//double tse= dst.at< double>(0, 0);
	//int tse2 = dst2.at< double>(0, 0);

	cv::Mat planes[2] = { Mat_real, Mat_imag };
	cv::Mat dst;
	merge(planes, 2, dst);



	cudaMemcpy(&result[0], dst.data, dataH * dataW * Nz * sizeof(cufftDoubleComplex), cudaMemcpyHostToDevice);
...
void CalFuntion::FFT2D_1(cufftDoubleComplex* dst, cufftDoubleComplex* src, sCudaPara& para, int inverse)
{
	...
	if (inverse == 1)
	{
		...
		hengxiang = 0;
		zongxiang = 0;
		ShowComplex(dst, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
...
void CalFuntion::otf2psf(double* psf, double* otf,int ImgSize,  sCudaPara& para)
{
	double* TempRealOne;//建立，释放
	cudaMalloc((void**)&TempRealOne, (ImgSize * ImgSize) * sizeof(double));
	fftShfit_R << <blocks, threads >> > (TempRealOne, otf);
	cufftDoubleComplex* CTempOne;//建立，释放
	cudaMalloc((void**)&CTempOne, ImgSize * ImgSize * sizeof(cufftDoubleComplex));
	...
	Double2Complex << <blocks, threads >> > (CTempOne, TempRealOne);
	cufftExecZ2Z(fft_loco, CTempOne, CTempOne, 1);
	ifftR2RRec << <blocks, threads >> > (CTempOne, CTempOne, ImgSize);
	Complex2Double << <blocks, threads >> > (TempRealOne, CTempOne, ImgSize);
	

	fftShfit_R << <blocks, threads >> > (psf, TempRealOne);
	cudaFree(CTempOne);
	cudaFree(TempRealOne);
	cufftDestroy(fft_loco);

	Abs_R << <blocks, threads >> > (psf, psf);

}

void CalFuntion::importImages(double* dst, double* src, sCudaPara& para)
{
	double fac = (1 / 10.0) * pi_Loco / 2.0;
	fadeBorderCos << <blocks2, threads >> > (dst, src, fac, 10);


}

void CalFuntion::find_illumination_pattern(double* peak_kx, double* peak_ky, double* Snoisy, double SIMparams_SIMpixelsize, 
	cufftDoubleComplex* dst, double* Module, sCudaPara& para)
{
	...
	hengxiang = 245;
	zongxiang = 255;
	ShowDouble(otf, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
	double* psf;//建立，未开辟

	cudaMalloc((void**)&psf, ImgSizeBefore * ImgSizeBefore * sizeof(double));

	otf2psf(psf, otf, ImgSizeBefore,para);

	printf("psf :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowDouble(psf, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
	...
	double* TempTwo;//建立，释放
	cudaMalloc((void**)&TempTwo, 15 * ImgSizeBefore * ImgSizeBefore * sizeof(double));

	importImages(IIraw, Snoisy, para);

	printf("Line 1002 Temp=importImages(Snoisy) :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowDouble(IIraw, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 15);
	deconvlucy(TempTwo, IIraw, psf,ImgSizeBefore, ImgSizeBefore,15);//结果转置

	printf("Line 1014 IIraw_conj:***************************************\n");
	hengxiang = 10;
	zongxiang = 10;
	ShowDouble(TempTwo, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1 * 15);




	Matrixconj << <blocks, threads >> > (IIraw, TempTwo, ImgSizeBefore, 15);//转置，结果正确

	cudaFree(TempTwo);


	printf("Line 1014 IIraw :***************************************\n");
	hengxiang = 10;
	zongxiang = 10;
	ShowDouble(IIraw, ImgSizeBefore , ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1 * 15);
	...
	printf("Line 1015 IIrawFFT :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowComplex(IIrawFFT, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1 * 15);
	...
	printf("Line 1029 NotchFilter2 :***************************************\n");
	hengxiang = 92;
	zongxiang = 305;
	ShowDouble(NotchFilter2, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
	...
	double* TempOne;//建立，释放
	cudaMalloc((void**)&TempOne, ImgSizeBefore* ImgSizeBefore * sizeof(double));
	double* TempThree;//建立，释放
	cudaMalloc((void**)&TempThree, ImgSizeBefore* ImgSizeBefore * sizeof(double));
	...
	for (int i = 0; i < 3; i++)//必须是3而非Nz
	{
		...
		hengxiang = 0;
		zongxiang = 0;
		ShowComplex(c3_Mask, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
		...
		MatrixReal << <blocks, threads >> > (TempOne, c3_Mask, ImgSizeBefore);

		printf("Line 1049 c3_real :***************************************\n");
		hengxiang = 250;
		zongxiang = 250;
		ShowDouble(TempOne, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 8, hengxiang + 0, hengxiang + 8, 0, 1);

		MatrixImag << <blocks, threads >> > (TempThree, c3_Mask, ImgSizeBefore);

		Mat_FFT2D_1(c3_Mask, TempOne, TempThree, ImgSizeBefore, ImgSizeBefore,1);

		printf("Line 1053 FFTC3 :***************************************\n");
		hengxiang = 0;
		zongxiang = 0;
		ShowComplex(c3_Mask, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
		...
		printf("Line 1054 vec :***************************************\n");
		hengxiang = 232;
		zongxiang = 63;
		ShowComplex(vec, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);

		CudaMultyComplexWithDouble << <blocks, threads >> > (CTemp, vec, NotchFilter2, ImgSizeBefore);
		Cal_Temp << <blocks, threads >> > (Temp, CTemp, ImgSizeBefore);
		printf("Line 1057 temp :***************************************\n");
		hengxiang = 92;
		zongxiang = 305;
		ShowDouble(Temp, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
...
void CalFuntion::process_data(cufftDoubleComplex* dst1, double* dst2, double* kx, double* ky, 
	double* allimages_in, cufftDoubleComplex* OTFem, cufftDoubleComplex* find_illumination_pattern_p1, double* Module, sCudaPara& para)//有冗余优化空间，Testj循环三次，三次计算相同取不同维度，extend。
{
	...
	for (int Testi2 = 0; Testi2 < 3; Testi2++)//必须是3,不是Nz
	{
		...
		printf("mixing_matrix :***************************************\n");
		hengxiang = 0;
		zongxiang = 0;
		ShowComplex(mixing_matrix, 5, 5, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);


		Inv(unmixing_matrix, mixing_matrix, para);

		printf("unmixing_matrix :***************************************\n");
		hengxiang = 0;
		zongxiang = 0;
		ShowComplex(unmixing_matrix, 5, 5, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1);
		...
		printf("allftorderims_up :***************************************\n");
		hengxiang = 510;
		zongxiang = 510;
		ShowComplex(allftorderims, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 5);
...
void CalFuntion::Recon3DSIM(cv::Mat* SimResult, sCudaPara& para, int dataH, int dataW)
{
	...
	if (para.dataparamsCpu.Nz != 1)
	{
		...
		/*cout << "Line1963:" << endl;
		ShowComplex(OTFem, 1024, 1024, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, para.dataparamsCpu.Nz);*/
	...
	Cal_MCNR(MCNR, para.dataparamsCpu.images, dataH, dataW,para.dataparamsCpu.Nz);
	//averageMCNR_foreground_top << <blocks_loco, threads >> > (MCNR, para.dataparamsCpu.Nz);

	printf("MCNR :***************************************\n");
	ShowDouble(MCNR, 1, para.dataparamsCpu.Nz, 0, 1, 0, para.dataparamsCpu.Nz, 0, 1);
	...
	printf("Line 1839 Snoisy :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowDouble(Snoisy, ImgSizeBefore, ImgSizeBefore, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 15);
	...
	hengxiang = 0;
	zongxiang = 0;
	ShowDouble(Module, 1, 5, zongxiang, zongxiang + 1, hengxiang + 0, hengxiang + 5, 0, 1);

	printf("find_illumination_pattern_p1 :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowComplex(find_illumination_pattern_p1, 1, 6, zongxiang, zongxiang + 1, hengxiang + 0, hengxiang + 6, 0, 1);
	...
	hengxiang = 460;
	zongxiang = 460;
	ShowComplex(sum_fft, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, para.dataparamsCpu.Nz);
	...
	//hengxiang = 491;
	//zongxiang = 197;
	//ShowDouble(Filter1, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, para.dataparamsCpu.Nz);
	//printf("Filter2 :***************************************\n");
	//hengxiang = 0;
	//zongxiang = 0;
	//ShowDouble(Filter2, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, para.dataparamsCpu.Nz);
	//for (int jNz = 0; jNz < para.dataparamsCpu.Nz; jNz++)
	//{
	...
	zongxiang = 460;
	ShowComplex(sum_fft, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, para.dataparamsCpu.Nz);
	//(double* result, std::vector<cv::Mat>&Snoisy, int numpixelsx1, int Nz, double Na, double emwavelength, double rawpixelsize);
	...
	double* Hilo;
	cudaMalloc((void**)&Hilo, (para.dataparamsCpu.Nz * ImgSizeAfter * ImgSizeAfter) * sizeof(double));
	Mat_HiloGen(Hilo, para.dataparamsCpu.images, ImgSizeBefore, para.dataparamsCpu.Nz, para.dataparamsCpu.NA, para.dataparamsCpu.emwavelength, para.dataparamsCpu.rawpixelsize[0]);
	printf("Hilo :***************************************\n");
	hengxiang = 0;
	zongxiang = 0;
	ShowDouble(Hilo, ImgSizeAfter, ImgSizeAfter, zongxiang, zongxiang + 5, hengxiang + 0, hengxiang + 5, 0, 1 * para.dataparamsCpu.Nz);
	double* final_image;
	cudaMalloc((void**)&final_image, (para.dataparamsCpu.Nz * ImgSizeAfter * ImgSizeAfter) * sizeof(double));
	for (int jNz = 0; jNz < para.dataparamsCpu.Nz; jNz++)
	{
		Calfinal_image << <blocks7, threads >> > (final_image + jNz * ImgSizeAfter * ImgSizeAfter,
			Hilo + jNz * ImgSizeAfter * ImgSizeAfter, Result + jNz * ImgSizeAfter * ImgSizeAfter, ImgSizeAfter);
	}
	...
	for (int ti = 0; ti < para.dataparamsCpu.Nz; ti++)
	{
		...
		h_Result = (double*)malloc(dataH * dataW * sizeof(double));
		//拷贝至内存
		cudaMemcpy(h_Result, &final_image[ti * dataH * dataW], dataH * dataW * sizeof(double), cudaMemcpyDeviceToHost);
		//赋值给cv::Mat并打印
		cv::Mat_<double> resultReal = cv::Mat_<double>(dataH, dataW);
		cv::Mat_<double> resultImag = cv::Mat_<double>(dataH, dataW);
		for (int i = 0; i < dataH; i++) {
